...
// --- THUẬT TOÁN TÍNH ÂM LỊCH (Giữ nguyên logic của bạn) ---
class LunarCalendar {
...
export class Ex10 {
  ...
  convert() {
    // QUAN TRỌNG: Ép kiểu về số (Number) để tránh lỗi cộng chuỗi gây ra số khổng lồ
    ...
    const lunarYearName = `${cans[canIndex]} ${chis[chiIndex]}`;

    // Format hiển thị
    const thangText = lunarLeap ? `Tháng ${lunarMonth} (nhuận)` : `Tháng ${lunarMonth}`;
    const amLichText = lunarLeap 
    ...
    this.result = {
      ...
      duongLich: `${dd}/${mm}/${yy}`,
      amLich: amLichText,
      namAm: `Năm ${lunarYearName}`,
      thangAm: thangText,
      ngayAm: `Ngày ${lunarDay}`
    };
  }
}